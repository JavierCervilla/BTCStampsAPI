...
export class Src20Controller {
		...
		try {
			...
			if (balanceParams.includePagination) {
				const fetchedTotalCount =
					await Src20Service.getTotalSrc20BalanceCount(balanceParams);
				const limit =
					balanceParams.limit ||
	...
	static async handleWalletBalanceRequest(
		...
		try {
			...
			const [btcInfo, stampsResponse, src20Response, lastBlock] =
			...
			const stampsData =
				stampsResponse.status === "fulfilled"
					? stampsResponse.value
					: { stamps: [], total: 0 };
			const src20Data =
				src20Response.status === "fulfilled"
					? src20Response.value
					: { data: [], last_block: 0 };
			const lastBlockData =
				lastBlock.status === "fulfilled" ? lastBlock.value : null;
	...
	static async handleTickPageRequest(tick: string) {
		...
		const [balanceResponse, mintProgressResponse, allSrc20DataResponse] =
			await Promise.all([
