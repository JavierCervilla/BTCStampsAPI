...
export class StampController {
	...
	private static calculatePrices(
		...
		if (dispenses && dispenses.length > 0 && stamp.supply) {
			...
			const recentPrice =
				mostRecentDispense.dispenser_details.satoshirate / 100000000;
		...
		try {
			...
			if (!finalIdent) {
				if (filterByArray.length > 0) {
					finalIdent = filterByArray.flatMap(
						(filter) => filterOptions[filter]?.ident || [],
					);
			...
			const updatedStamps = await Promise.all(
				stampResult.stamps.map(async (stamp: StampRow) => {
					const xcpAsset = xcpAssets.find(
						(asset) => asset.asset === stamp.cpid,
					);
