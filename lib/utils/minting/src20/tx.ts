...
export const prepareSrc20TX = async ({
	...
	try {
		...
		// Add fee if present
		if (fee && recipientFee) {
			vouts.push({ address: recipientFee, value: Number.parseInt(fee) });
		}

		// Add minting service fee if enabled
		if (
			Number.parseInt(serverConfig.MINTING_SERVICE_FEE_ENABLED || "0") === 1
		) {
			const feeAddress = serverConfig.MINTING_SERVICE_FEE_ADDRESS || "";
			const feeAmount = Number.parseInt(
		...
		for (const input of selectedUtxos) {
			...
			const isWitnessUtxo =
				inputDetails.scriptPubKey.type.startsWith("witness");
