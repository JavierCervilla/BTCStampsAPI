...
export class SRC20Repository {
	static async getTotalCountValidSrc20TxFromDb(params: SRC20TrxRequestParams) {
		...
		if (tick !== null) {
			if (Array.isArray(tick)) {
				whereConditions.push(`tick IN (${tick.map(() => "?").join(", ")})`);
	...
	static async getValidSrc20TxFromDb(params: SRC20TrxRequestParams) {
		const {
			block_index,
			tick,
			op,
		...
		if (tick !== undefined) {
			if (Array.isArray(tick)) {
				...
				whereConditions.push(`src20.tick IN (${tickPlaceholders})`);
				queryParams.push(...tick);
			} else {
	...
	static async getSrc20BalanceFromDb(
		...
		if (tick) {
			if (Array.isArray(tick)) {
				const tickPlaceholders = tick.map(() => "?").join(", ");
				whereClauses.push(`tick IN (${tickPlaceholders})`);
				queryParams.push(...tick);
			} else {
  ...
  `;
	...
	static async getTotalSrc20BalanceCount(
		...
		if (tick) {
			if (Array.isArray(tick)) {
				const tickPlaceholders = tick.map(() => "?").join(", ");
				whereConditions.push(`tick IN (${tickPlaceholders})`);
				queryParams.push(...tick);
			} else {
				whereConditions.push(`tick = ?`);
				queryParams.push(tick);
	...
	static async getSrc20MintProgressByTickFromDb(tick: string) {
		const query = `
        SELECT 
            src.max,
            src.deci,
