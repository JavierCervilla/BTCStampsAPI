...
import { SRC20_TABLE, STAMP_TABLE } from "utils/constants.ts";
import { dbManager } from "../../server/database/db.ts";

const BLOCK_FIELDS = `block_index, block_time, block_hash, previous_block_hash, ledger_hash, txlist_hash, messages_hash`;
...
export class BlockRepository {
	 ...
	 */
	static async getBlockInfoFromDb(blockIdentifier: number | string) {
		const isIndex =
	...
	static async getRelatedBlocksWithStampsFromDb(
		...
		if (typeof blockIdentifier === "number" || /^\d+$/.test(blockIdentifier)) {
			...
			block_index = await this._getBlockIndexByHash(String(blockIdentifier));
		}
	...
	static async countStampsInBlock(block_index: number | string) {
		const isIndex =
			typeof block_index === "number" || /^\d+$/.test(block_index);
		const field = isIndex ? "block_index" : "block_hash";
		const queryValue = isIndex ? Number(block_index) : block_index;
